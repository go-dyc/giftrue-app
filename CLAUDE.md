...
- **한국어 최적화**: 90-110자 한국어 기념패 문구 생성에 특화 (2025-07-25 업데이트)
...
**개발 상태**: ✅ AI 문구 생성 최적화 + 사용자 경험 개선 + 한글 검증 메시지 완료  
**주요 URL**: `/orders/{naver_order_number}` (고객용), `/admin` (관리자용), `/orders/{order_id}/generate_content` (AI API)

**최신 개선사항** (2025-07-25):
- 📝 **AI 문구 생성 개선**: 드롭다운/라디오 → 직접 입력 + 추천 태그 방식으로 UI 개선
- 🔢 **문구 길이 최적화**: AI 생성 문구 150자 → 110자로 조정하여 기념패 가독성 향상  
- 🇰🇷 **한글 검증 메시지**: 모든 에러 메시지를 영어에서 한글로 완전 번역
- 🤖 **맥락 기반 생성**: 관계(주는이→받는이), 목적, 톤을 직접 입력으로 더 정확한 문구 생성
- 💾 **데이터 지속성**: 커스텀 입력 필드 값이 수정 페이지에서 정상 유지되도록 개선
...
**현재 커밋**: `a5ec168` (2025-07-25 AI 문구 생성 UI 개선 + 110자 제한 + 한글 검증 메시지)  
**GitHub 저장소**: https://github.com/go-dyc/giftrue-app