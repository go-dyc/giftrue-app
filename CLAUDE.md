...
- `relationship_giver`: string, 관계 주는이 (직접 입력, 예: 상사, 동료, 회사)
- `relationship_receiver`: string, 관계 받는이 (직접 입력, 예: 부하직원, 동료, 개인)
- `purpose_custom`: string, 목적/상황 직접 입력 
- `tone_custom`: string, 문구 스타일 직접 입력
...
### AI 문구 생성 시스템 (2025-07-25 완전 개편)
- **Google Gemini 1.5 Flash**: gemini-ai gem을 통한 연동 (Temperature 0.8, MaxTokens 400)
- **단순화된 프롬프트**: 사용자 입력값을 직접 활용한 자연어 프롬프트 ("김현호님, 상사→부하, 퇴직을 위한 기념패 문구를 따뜻하게 작성해주세요")
- **12자 줄바꿈**: 기념패에 최적화된 가독성 좋은 줄바꿈 자동 적용
- **완전한 문장**: 400 토큰으로 문장이 중간에 끊어지지 않도록 보장
- **실시간 생성**: 비동기 JavaScript로 즉시 문구 생성 및 적용
- **극단적 단순화**: 복잡한 템플릿 시스템 완전 제거, 6개 핵심 메서드만 유지

### 핵심 검증 시스템
- **단계별 검증**: 각 단계마다 필요한 필드만 검증
- **클라이언트 사이드**: 실시간 검증, 에러 표시, 자동 스크롤
- **서버 사이드**: 모델 레벨 검증, 파일 형식/크기 제한
- **한글 에러 메시지**: 모든 검증 메시지 한국어 지원 (2025-07-25 업데이트)
...
- ✅ **한국어 최적화**: 90-110자 기념패 문구 생성에 특화 (2025-07-25 업데이트)
...
**문서 최종 업데이트**: 2025-07-25  
**개발 상태**: ✅ AI 프롬프트 완전 단순화 + 스타일별 정보 표시 시스템 완료  
**주요 URL**: `/orders/{naver_order_number}` (고객용), `/admin` (관리자용), `/orders/{order_id}/generate_content` (AI API)

**최신 개선사항** (2025-07-25 오후):
- 🤖 **AI 프롬프트 완전 단순화**: 복잡한 템플릿 시스템 제거, 사용자 입력값 직접 활용한 자연어 프롬프트
- 🧹 **코드 대폭 정리**: 10개 이상 헬퍼 메서드 → 6개 핵심 메서드로 90% 감소
- 📝 **직관적 프롬프트**: "전역패, 김현호님, 상사→부하, 퇴직, 20년 성실근무를 위한 기념패 문구를 따뜻하고 감동적으로 작성해주세요"
- 🎯 **스타일별 정보 표시**: 주문 완료/관리자 페이지에서 선택한 기념패 스타일 관련 필드만 표시
- 🔧 **Order 모델 확장**: 스타일별 필드 존재 여부 확인 로직 추가 (`metal_plaque?`, `has_ai_generation_info?` 등)
- 💾 **데이터 보존**: 모든 기존 입력값 유지하되 화면에서는 관련 정보만 표시
- ⚡ **성능 최적화**: AI 모델 설정 개선 (Temperature 0.8, MaxTokens 400, 완전한 문장 보장)

**AI 프롬프트 시스템 개선**:
- ❌ **제거**: 목적별 복잡한 템플릿 (퇴직/승진/졸업 등)
- ❌ **제거**: 헬퍼 메서드들 (`get_relationship_description`, `get_tone_guide` 등)
- ✅ **추가**: 사용자 입력 직접 활용 (relationship, purpose, tone 그대로 사용)
- ✅ **추가**: 12자씩 자연스러운 줄바꿈 처리
- ✅ **추가**: 완전한 문장으로 마무리 보장

**이전 개선사항** (2025-07-24 저녁):
...
**현재 커밋**: 진행 중 (2025-07-25 AI 프롬프트 완전 단순화 + 스타일별 정보 표시)  
**GitHub 저장소**: https://github.com/go-dyc/giftrue-app

**핵심 파일 경로**:
- AI 프롬프트: `app/services/gemini_service.rb` (154줄, 6개 메서드)
- Order 모델: `app/models/order.rb` (스타일별 표시 로직 포함)
- 주문 완료: `app/views/orders/complete.html.erb` (조건부 필드 표시)
- 관리자 상세: `app/views/admin/orders/show.html.erb` (스타일별 섹션)